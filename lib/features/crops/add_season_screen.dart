...
class _AddSeasonScreenState extends ConsumerState<AddSeasonScreen> {
  ...
  void _onVarietyChanged(String? variety) {
    setState(() {
      ...
      _targetYield =
          variety != null ? CropConstants.defaultYield(variety) : null;
      if (_plantingDate != null) {
        _harvestDate = _plantingDate!
            .add(const Duration(days: CropConstants.seasonDurationDays));
      }
    });
  }
  ...
  void _onPlantingDateChanged(DateTime date) {
    setState(() {
      _plantingDate = date;
      _harvestDate =
          date.add(const Duration(days: CropConstants.seasonDurationDays));
    });
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(seasonsProvider.notifier).addSeason({
        'farmer_id': _selectedFarmerId,
        'variety': _selectedVariety,
        'planting_date': _plantingDate,
        'harvest_date': _harvestDate,
        'target_yield_t_ha': _targetYield ?? 30.0,
        'plot_id': null,
      });
      if (!mounted) return;
      ...
      if (!mounted) return;
      _showSnack('Failed to save season. Please try again.');
    } finally {
              ...
              children: [
                ...
                Padding(
                  ...
                  child: AppCard(
                    ...
                    child: farmersAsync.when(
                      ...
                      error: (e, _) => Text('Could not load farmers: $e'),
                      data: (farmers) {
                        final activeFarmers =
                            farmers.where((f) => !f.isDeleted).toList();
                ...
                Padding(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(_loading ? 'Saving...' : 'Start Season'),
                  ),
                ),
